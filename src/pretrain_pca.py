...
def pretrain(args):
    ...
    g_encoder, m_predictor = pretrain_moldescpred(g_encoder, m_predictor, train_loader, pc_eigenvalue, args.seed)
    ...
    pc_eigenvalue = torch.from_numpy(pretraining_dataset.pc_eigenvalue).to(cuda)
...
def pretrain_moldescpred(
    ...
    pretrained_model_path = "./model/pretrained/" + "%d_pretrained_gnn_pca.pt" % (seed)
    ...
    #plt.savefig("graph/pca_train_loss_ " + str(len(trn_loss_list)) + "_epochs.png")
...
class Pretraining_Dataset:
    ...
    def load(self, mode):
        ...
        plt.grid(True)
        plt.show()
        plt.savefig("explained_variance" + str(len(explained_variance_ratio)) + ".png")
