...
#Standard loss formula added by Lama due to ISOMAP having no eigenvalues
...
def pretrain_moldescpred(
    ...
    pretrained_model_path = "./model/pretrained/" + "%d_pretrained_gnn_isomap.pt" % (seed)
...
class Pretraining_Dataset:
    ...
    def load(self, mode):
        ...
        # Applying Isomap to reduce the dimensionality of descriptors
        isomap = Isomap(n_components=self.pc_num)
        mordred = isomap.fit_transform(mordred)
